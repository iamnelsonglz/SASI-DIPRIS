...
{
    global $mysqli;
    $select_query = "SELECT username, Usuario.nombre, apellidoPaterno, apellidoMaterno, Categoria.nombre AS categoria
    FROM Usuario 
    INNER JOIN Categoria ON Usuario.categoria = Categoria.idCategoria
    WHERE not username = 'root' ORDER BY Usuario.nombre;";
    $result = $mysqli->query($select_query);
    if (!$result) {
        die('Error de consulta');
    }
