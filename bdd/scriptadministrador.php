...
else if (isset($_GET['listadosoporte'])){
    listSoporte();
}
...
// Condición para mostrar todos los estados en el select
    ...
    $estado = $_GET['estado'];
    previsualizarSolicitud($folio, $estado);
}
...
function listSoporte(){
    global $mysqli;
    $select_query = "SELECT nombre FROM Usuario WHERE categoria = '2' ORDER BY nombre ASC";
    $result = $mysqli->query($select_query);
    if (!$result) {
        echo "No se encontro resultados";
    }
...
function previsualizarSolicitud($folio, $estado)
{
    global $mysqli;

    switch ($estado) {
        case 1:
            $query = "SELECT folio, usuario.nombre AS nombre, usuario.apellidoPaterno AS paterno, usuario.apellidoMaterno AS materno, 
        ...
        break;
        case 2:
            $query = "SELECT folio, usuarioa.nombre AS nombre, usuarioa.apellidoPaterno AS paterno, usuarioa.apellidoMaterno AS materno, 
                usuariob.nombre AS responde, categoria.nombre AS categoria, departamento.nombre AS departamento, departamento.planta AS planta,
                descripcion FROM reporte
                ...
                INNER JOIN Departamento ON Usuarioa.departamento = Departamento.idDepartamento
                WHERE folio = '$folio'";
            $result = $mysqli->query($query);
            if (!$result) {
                echo "";
        ...
        break;
        case 3:
            $query = "SELECT folio, descripcion, respuesta FROM reporte WHERE folio = '$folio'";
            $result = $mysqli->query($query);
            if (!$result) {
                echo "";
            } else {
...
{
    global $mysqli;
    $select_query = "SELECT folio, fecha_respuesta AS fecha, tipo_reporte.nombre AS tipo, Usuarioa.nombre AS usuario, Usuariob.nombre AS responde FROM reporte
    INNER JOIN Tipo_reporte ON reporte.tipo = tipo_reporte.idTipo
    INNER JOIN usuario AS Usuarioa ON Reporte.usuario_reporta = Usuarioa.username
    INNER JOIN usuario AS Usuariob ON Reporte.usuario_responde = Usuariob.username
    INNER JOIN Categoria ON Usuarioa.categoria = Categoria.idCategoria
    WHERE estado = '3'  AND fecha_respuesta >= CURDATE()
    ORDER BY Usuarioa.categoria ASC, Tipo_reporte.prioridad ASC, fecha_respuesta DESC";
    $result = $mysqli->query($select_query);
    if (!$result) {
        echo "";
    } else {
...
{
    global $mysqli;
    $select_query = "SELECT folio, fecha_respuesta AS fecha, tipo_reporte.nombre AS tipo, usuario_reporta AS usuario FROM reporte
    INNER JOIN Tipo_reporte ON reporte.tipo = tipo_reporte.idTipo
    INNER JOIN usuario ON Reporte.usuario_reporta = Usuario.username
    INNER JOIN Categoria ON Usuario.categoria = Categoria.idCategoria
    WHERE estado = '4' AND fecha_respuesta >= CURDATE()
